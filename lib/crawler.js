    ...
    async.whilst(function () {
        ...
        that.request(uri, function (status, $) {
            if (status) {
                ...
                var content = eval(last.$).html();
                var word_content = eval(last.$).text();

                var resultArray = nodejieba.extract(word_content, 30);
                list = [];
...
Crawler.prototype.request = function (url, callback) {
    ...
    request(opts, function (err, res, body) {
        ...
        if (!err && res.statusCode == 200) {
            ...
            $ = cheerio.load(iconv.decode(body, config.charset || 'utf8'), {decodeEntities: false});
        } else {
